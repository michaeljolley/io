...
export const contributeCommand = async (
  ...
  if (args.length >= 3) {
    ...
    if (projectSettings && projectSettings.issue) {
      ...
      await post(url, { repo: projectSettings.repo, comment: comment, title: "Add new contributor" }).then((response: any) => {
          if (response.status === 201) {
            url = `http://api/issues/${response.data.number}/comment`;
          } else {
            sendMessage(twitchChatFunc, 'There was a problem adding the contributor');
            return true;
        }
      });
    }
