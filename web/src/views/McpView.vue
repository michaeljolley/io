  ...
  <div class="p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      <button
        ...
        class="btn-gradient inline-flex items-center gap-1"
      >
    ...
    <div v-if="showAdd" class="border border-border rounded-lg p-4 mb-6 space-y-3 bg-card">
      ...
      <div>
        ...
        />
        <p class="mt-1 text-xs text-muted-foreground">Use KEY=value lines, or paste JSON. Values like ${FIGMA_API_KEY} are preserved.</p>
      </div>
    ...
    <div v-else class="space-y-2">
      <div v-for="server in servers" :key="server.id" class="flex items-center justify-between border border-border rounded-lg px-4 py-3 bg-card">
        <div class="space-y-1">
          <div class="flex items-center gap-2">
            <span class="font-medium text-sm">{{ server.name }}</span>
            <span class="text-xs text-muted-foreground bg-secondary px-1.5 py-0.5 rounded">{{ server.type }}</span>
          </div>
          <div v-if="server.command || server.url" class="text-xs text-muted-foreground">{{ server.command || server.url }}</div>
          <div v-if="server.args?.length" class="text-xs text-muted-foreground">Args: {{ server.args.join(', ') }}</div>
          <div v-if="server.env && Object.keys(server.env).length > 0" class="text-xs text-muted-foreground">Env: {{ Object.keys(server.env).join(', ') }}</div>
        </div>
