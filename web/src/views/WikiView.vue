...
import { BookOpen, Search, Plus, Pencil, Trash2, Save, X, FileCode } from "lucide-vue-next";
...
async function saveTemplate() {
  ...
  await apiPut(`/wiki/template/squad/${selectedTemplate.value}`, { content: templateEditContent.value });
...
<template>
  <div class="flex flex-col h-full">
    ...
    <div class="flex border-b border-border shrink-0">
      <button
        @click="mode = 'pages'"
        :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors', mode === 'pages' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground']"
      >
      ...
      <button
        ...
        :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors', mode === 'templates' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground']"
      >
    ...
    <div v-else class="flex flex-1 overflow-hidden">
      ...
      <div class="w-64 border-r border-border flex flex-col shrink-0">
        <div class="p-3 border-b border-border space-y-2">
          ...
          <div v-if="showNewTemplateForm" class="space-y-1.5">
            ...
            <div class="flex gap-1">
              ...
              <button
                @click="showNewTemplateForm = false; newTemplatePath = ''"
                class="px-2 py-1 text-xs rounded border border-border hover:bg-accent"
              >
                Cancel
              </button>
        ...
        <div class="flex-1 overflow-y-auto p-2">
          ...
          <div v-else-if="templates.length === 0" class="text-xs text-muted-foreground p-2">No templates yet.</div>
          <button
            v-else
            v-for="t in templates"
            :key="t"
            @click="selectTemplate(t)"
            :class="['w-full text-left px-2 py-1.5 text-xs rounded hover:bg-accent font-mono truncate', selectedTemplate === t ? 'bg-accent text-accent-foreground' : 'text-foreground']"
          >
            {{ t }}
          </button>
        </div>
      ...
      <!-- Template content -->
      <div class="flex-1 flex flex-col overflow-hidden">
        <div v-if="!selectedTemplate" class="flex items-center justify-center h-full text-muted-foreground">
          <div class="text-center">
          ...
          <div class="flex items-center justify-between px-4 py-2 border-b border-border">
            ...
            <div class="flex gap-1">
              <template v-if="!templateEditMode">
                <button @click="startTemplateEdit" class="p-1.5 rounded hover:bg-accent text-muted-foreground" title="Edit">
                  <Pencil class="w-4 h-4" />
                </button>
                <button @click="deleteTemplate" class="p-1.5 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive" title="Delete">
                  <Trash2 class="w-4 h-4" />
                </button>
              </template>
              <template v-else>
                <button @click="saveTemplate" class="p-1.5 rounded hover:bg-accent text-green-500" title="Save">
                  <Save class="w-4 h-4" />
                </button>
                <button @click="templateEditMode = false" class="p-1.5 rounded hover:bg-accent text-muted-foreground" title="Cancel">
                  <X class="w-4 h-4" />
                </button>
              </template>
            </div>
