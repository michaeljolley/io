	...
	return [
		...
		defineTool("wiki_read", {
			...
			parameters: z.object({
				path: z
					.string()
					.describe("Page path relative to pages/ (e.g., 'notes/todo.md')"),
			}),
			handler: async ({ path }) => {
		...
		defineTool("squad_create", {
			...
			parameters: z.object({
				...
				universe: z
					.string()
					.describe(
						"Pop culture universe theme (e.g., 'A-Team', 'Transformers', 'ThunderCats')",
					),
				repo_url: z
					.string()
					.optional()
					.describe("Git repository URL for the project"),
			}),
		...
		defineTool("squad_add_agent", {
			...
			parameters: z.object({
				...
				character_name: z
					.string()
					.describe("Character name from the squad's universe"),
				role_title: z
					.string()
					.describe("Specialist role (e.g., 'Vue 3 Frontend Developer')"),
				persona: z
					.string()
					.optional()
					.describe("Personality/work style description"),
				is_lead: z.boolean().optional().describe("Is this the team lead?"),
				is_qa: z.boolean().optional().describe("Is this a QA reviewer?"),
				is_test: z
					.boolean()
					.optional()
					.describe("Is this a test/quality specialist?"),
			}),
			handler: async ({
				squad_id,
				character_name,
		...
		defineTool("squad_delegate", {
			...
			parameters: z.object({
				...
				instance_id: z
					.string()
					.optional()
					.describe("Instance ID (for parallel work)"),
			}),
			handler: async ({ squad_id, task, instance_id }) => {
				const { delegateTask } = await import("./agents.js");
				const { getActiveMessageAttachments } = await import(
					"./orchestrator.js"
				);
				...
				delegateTask(squad_id, task, instance_id, attachments).catch((err) => {
					...
					addAuditEntry(
						...
						`Background delegation failed: ${errMsg}`,
						{ squad_id, error: errMsg },
						{ squad_id },
					);
				});
		...
		defineTool("squad_meeting", {
			...
			parameters: z.object({
				...
				task: z
					.string()
					.describe("Detailed task description for the team to plan"),
				execute_after: z
					.boolean()
					.describe(
						"If true, execute the plan immediately after the meeting. If false, post plan to feed and wait for user approval.",
					),
			}),
			handler: async ({ squad_id, task, execute_after }) => {
				...
				squadMeeting(squad_id, task, execute_after, attachments).catch(
					(err) => {
						const errMsg = err instanceof Error ? err.message : "Unknown error";
						addAuditEntry(
							"squad_meeting_error",
							`Background meeting failed: ${errMsg}`,
							{ squad_id, error: errMsg },
							{ squad_id },
						);
					},
		...
		defineTool("squad_instance_create", {
			description:
				"Create a new parallel instance (worktree) for a squad. Max 3 per squad.",
			parameters: z.object({
				squad_id: z.string().describe("Squad ID"),
		...
		defineTool("squad_delete", {
			description:
				"Delete an entire squad and all its agents, tasks, and instances",
			parameters: z.object({
		...
		defineTool("feed_post", {
			...
			parameters: z.object({
				source: z
					.string()
					.describe("Source identifier (e.g., 'orchestrator', 'squad-alpha')"),
		...
		defineTool("schedule_create", {
			...
			parameters: z.object({
				...
				agenda: z
					...
					.optional()
					.describe(
						"Agenda type for squad schedules (triage, prioritize, ideation, or custom)",
					),
		...
		defineTool("schedule_update", {
			description:
				"Update an existing schedule's cron, agenda, prompt, or enabled status",
			parameters: z.object({
		...
		defineTool("shell_exec", {
			...
			handler: async ({ command, cwd }) => {
				...
				const { buildShellEnv } = await import("./shell-env.js");
				const execAsync = promisify(exec);
				...
				try {
					...
					addAuditEntry("shell_command", `Command: ${command.slice(0, 200)}`, {
						command,
						cwd,
