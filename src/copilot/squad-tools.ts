 ...
 */
export function createSquadTools(
	squadSlug: string,
	squadId: string,
	workDir: string,
): Tool<any>[] {
	...
	return [
		...
		defineTool("wiki_read", {
			...
			parameters: z.object({
				path: z
					.string()
					.describe(
						"Page path (e.g., 'decisions.md', 'notes/architecture.md')",
					),
			}),
			handler: async ({ path }) => {
		...
		defineTool("wiki_write", {
			description:
				"Write or update a wiki page in the squad wiki (paths are relative — do NOT include the squad prefix)",
			parameters: z.object({
		...
		defineTool("file_read", {
			...
			parameters: z.object({
				path: z
					.string()
					.describe("File path relative to project root (e.g., 'src/main.ts')"),
				start_line: z
					.number()
					.optional()
		...
		defineTool("file_search", {
			...
			parameters: z.object({
				...
				glob: z
					...
					.optional()
					.describe(
						"File glob to filter (e.g., '*.cs', '*.ts'). Searches all files if omitted.",
					),
				path: z
					.string()
					.optional()
					.describe(
						"Subdirectory to search in, relative to project root (optional)",
					),
				max_results: z
					.number()
					.optional()
		...
		defineTool("file_find", {
			...
			parameters: z.object({
				pattern: z
					.string()
					.describe(
						"File name pattern (e.g., 'DockWindow.xaml.cs', '*.test.ts', 'package.json')",
					),
				path: z
					.string()
					.optional()
		...
		defineTool("shell_exec", {
			...
			handler: async ({ command, cwd }) => {
				...
				try {
					// Inject GitHub credentials for git/gh CLI operations
	...
	return [
		defineTool("delegate_to_specialist", {
			...
			parameters: z.object({
				agent_name: z
					.string()
					.describe(
						"Character name of the specialist to delegate to (from your team roster)",
					),
				sub_task: z
					.string()
					.describe(
						"Detailed description of the sub-task. Be specific about what to implement, which files to touch, acceptance criteria, and branch to work on.",
					),
			}),
			handler: async ({ agent_name, sub_task }) => {
				...
				const agent = agents.find(
					(a) =>
						a.character_name.toLowerCase() === agent_name.toLowerCase() &&
						!a.is_lead,
				);
		...
		defineTool("delegate_to_specialists_parallel", {
				...
				assignments: z
					.array(
						z.object({
							agent_name: z
								.string()
								.describe("Character name of the specialist"),
			...
			handler: async ({ assignments }) => {
				const { getAgentsForSquad } = await import("../store/squads.js");
				const { runSpecialistsParallel } = await import(
					"./specialist-runner.js"
				);
				const agents = getAgentsForSquad(squadId);
					...
					const agent = agents.find(
						(a) =>
							a.character_name.toLowerCase() ===
								assignment.agent_name.toLowerCase() && !a.is_lead,
					);
					if (!agent) {
				...
				const preamble =
					errors.length > 0 ? `⚠️ Skipped: ${errors.join("; ")}\n\n` : "";
